...
<template>
  ...
  <div class="flex items-center gap-4">
    ...
    <div class="flex">
      ...
      <div v-for="(user, index) in store.users" :key="user" class="flex flex-col items-center"
        ...
        <div
          ...
          }" :title="user"><!-- Add tooltip with username on hover -->

          <!-- The jdenticon image -->
          <img :src="getUrl(user)" alt="avatar"
            class="w-full h-full object-contain rounded-full"><!-- Ensure image itself is rounded if needed -->
        </div>

        <!-- "You" label, shown only for the current user -->
        <span v-if="user === store.username" class="mt-1 text-xs text-red-600 font-medium">
